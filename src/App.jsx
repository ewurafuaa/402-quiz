...
/* ------------------------------------------------------------------
 * DATA SHAPE (every unit file must export this)
 * ------------------------------------------------------------------
 * Each unit file:  export default [ ...questions ]
 *
 * Each question:
 *   {
 *     id: 1,
 *     question: "Management means:",
 *     options: { A: "...", B: "...", C: "...", D: "..." }, // omit for True/False
 *     answer: "C"                                          // "A".."D", or "A"/"B" for T/F
 *   }
 * ------------------------------------------------------------------ */

console.log("[StudyApp] unit1 =", unit1);
console.log("[StudyApp] typeof unit1 =", typeof unit1);
console.log("[StudyApp] isArray =", Array.isArray(unit1));
console.log("[StudyApp] length =", unit1 && unit1.length);
...
const UNIT_SECTIONS = {
  unit1: [
    { id: "concepts",  label: "Management Concepts & Definitions",
      match: (t) => /management (means|is)|six m|six basic inputs|koontz|drucker|art and science|science and art|feature of management|all pervasive|group activity|output of the management/.test(t) },
    { id: "functions", label: "Management Functions",
      match: (t) => /planning|organising|organizing|staffing|directing|controlling|posdcorb|function of management|functions of management|resourcing|direction|control/.test(t) },
    { id: "levels",    label: "Levels of Management",
      match: (t) => /level of management|top level|middle level|lower level|supervisory level|top management|middle management|lower management/.test(t) },
    { id: "roles",     label: "Roles of a Manager (Mintzberg)",
      match: (t) => /mintzberg|interpersonal|informational|decisional|figurehead|liaison|disseminator|spokesperson|negotiator|resource allocator|disturbance handler|monitor role|leader role/.test(t) },
    { id: "skills",    label: "Managerial Skills",
      match: (t) => /technical skill|human skill|conceptual skill|design skill|four skills|skills of managers|top management requires|middle management requires|supervisors require/.test(t) },
    { id: "theories",  label: "Schools of Management Thought",
      match: (t) => /taylor|gilbreth|gantt|fayol|weber|mayo|follett|maslow|mcgregor|hawthorne|scientific management|bureaucratic|administrative management|human relation|theory x|theory y|systems approach|contingency approach|quantitative approach|modern management|classical theory|neo-classical|neoclassical|therbligs|piece-rate|time-and-motion|gantt chart/.test(t) },
    { id: "admin",     label: "Management vs Administration",
      match: (t) => /administration|management and administration|doing function|thinking function/.test(t) },
  ],
};

/* Normalize one raw question into the internal shape. */
function normalizeQuestion(raw, chapterId) {
  const hasOptions = raw && raw.options && typeof raw.options === "object";
  const opts = hasOptions
    ? ["A", "B", "C", "D"].map((k) => raw.options[k]).filter((v) => v !== undefined)
    : null;
  const isTF = !hasOptions;
  const kind = isTF ? "tf" : "mcq";

  let extra;
  if (isTF) {
    const truthy = raw.answer === "A" || raw.answer === true || raw.answer === "True";
    extra = { answer: truthy };
  } else {
    const idx = "ABCD".indexOf(String(raw.answer || "A").toUpperCase());
    extra = { options: opts, correct: idx >= 0 ? idx : 0 };
  }

  return {
    key: chapterId + "-" + raw.id,
    id: raw.id,
    chapter: chapterId,
    kind,
    text: raw.question || "",
    note: raw.note || raw.validation || "",
    ...extra,
  };
}

function buildChapter(meta) {
  const { raw, id, number, title } = meta;
  const sections = (UNIT_SECTIONS[id] || []).map((s) => ({ id: s.id, label: s.label }));
  const chapter = { id, number, title, sections };

  const questions = Array.isArray(raw) ? raw.map((q) => normalizeQuestion(q, id)) : [];

  const matchers = UNIT_SECTIONS[id] || [];
  questions.forEach((q) => {
    const t = q.text.toLowerCase();
    const hit = matchers.find((m) => m.match(t));
    q.section = hit ? hit.id : (sections[0] ? sections[0].id : "general");
  });

  if (questions.some((q) => q.section === "general") && !sections.find((s) => s.id === "general")) {
    sections.push({ id: "general", label: "General" });
  }
...
const CHAPTERS = [];
const BANK = [];
RAW_UNITS.forEach((meta) => {
  try {
    const { chapter, questions } = buildChapter(meta);
    CHAPTERS.push(chapter);
    BANK.push(...questions);
  } catch (e) {
    console.error("[StudyApp] buildChapter failed for", meta.id, e);
  }
});

console.log("[StudyApp] CHAPTERS =", CHAPTERS);
console.log("[StudyApp] BANK.length =", BANK.length);
...
const GLOBAL_CSS = `
  *, *::before, *::after { box-sizing: border-box; }
  ...
  /* Self-contained layout utilities so this component renders correctly
     even in a host app that doesn't have Tailwind CSS set up. */
  .flex { display: flex; }
  .grid { display: grid; }
...
function Home({ onPick, stats }) {
  ...
  const litCount = Math.round((stats.knownCount / total) * 64);
  return (
      ...
      <div className="flex flex-col md:flex-row md:items-end" style={{ gap: 40 }}>
        <div style={{ flex: 1 }}>
          <p style={{ fontFamily: MONO, fontSize: 12, color: C.gold, marginBottom: 14 }}>
            {CHAPTERS.length} units, {BANK.length} questions
          </p>
            ...
            {stats.history.slice(0, 5).map((h, i) => (
              <div key={i} className="flex items-center justify-between"
                ...
                <span style={{ fontFamily: SANS, fontSize: 14, color: C.body }}>
                  Unit {h.chapterNumber}, {h.sections.map(sectionLabel).join(", ").toLowerCase()}
                </span>
...
function ChapterPicker({ mode, onBack, onPick, known }) {
  return (
    <div className="mx-auto px-6 py-10" style={{ maxWidth: 760 }}>
      ...
      <h1 style={{ fontFamily: DISPLAY, fontSize: 34, fontWeight: 600, letterSpacing: -1, color: C.ink, margin: "16px 0 6px" }}>Pick a unit</h1>
      <p style={{ fontFamily: SANS, fontSize: 15, color: C.muted, marginBottom: 32 }}>
        {mode === "learn" ? "You can filter down to topics on the next screen." : "Test length and feedback come next."}
      </p>
        ...
        {CHAPTERS.map((ch) => {
          ...
          return (
            <button key={ch.id} onClick={() => onPick(ch)}
              ...
              <span>
                <span style={{ fontFamily: MONO, fontSize: 12, color: C.gold }}>Unit {ch.number}</span>
              ...
              <span style={{ fontFamily: MONO, fontSize: 12, color: C.gold, whiteSpace: "nowrap" }}>{ch.sections.length} topics</span>
            </button>
          );
        })}
      </div>
...
function Setup({ mode, chapter, onBack, onStart, saved }) {
  ...
  return (
    <div className="mx-auto px-6 py-10" style={{ maxWidth: 760 }}>
      <Button tone="ghost" size="sm" onClick={onBack} style={{ paddingLeft: 0 }}>All units</Button>
      ...
      <Field label="Repeated questions"
        hint="The bank repeats a lot of items word for word, especially in the later topics.">
        ...
        <div className="mx-auto px-6 flex items-center justify-between" style={{ maxWidth: 900, height: 56 }}>
          ...
          <span style={{ fontFamily: MONO, fontSize: 12, color: C.muted }}>
            {screen === "learn" ? "learning mode"
              ...
              : screen === "home" ? CHAPTERS.length + " units"
              : "unit " + chapter.number}
          </span>
        </div>
      </header>
